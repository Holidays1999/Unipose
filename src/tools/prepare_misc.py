...
from src.tools.eval_back import EvaluateCallBack
...
def prepare_callbacks(args, train_dataset, val_dataset, model):
    ...
    ckpoint_cb = ModelCheckpoint(prefix=f"unipose_{args.rank}", directory=args.ckpt_save_dir,
                                 config=config_ck)
    loss_cb = LossMonitor()
    eval_cb = EvaluateCallBack(model=model, eval_dataset=val_dataset, src_url=args.ckpt_save_dir,
