...
__all__ = ["multistep_lr", "cosine_lr", "constant_lr", "get_policy", "exp_lr", "poly_lr"]
...
def exp_lr(args, batch_num):
    ...
    def _lr_adjuster(epoch):
        if epoch < args.warmup_length:
            ...
            lr = args.base_lr * args.lr_gamma ** epoch + args.min_lr
...
def cosine_lr(args, batch_num):
    ...
    def _lr_adjuster(epoch):
        if epoch < args.warmup_length:
            ...
            lr = 0.5 * (1 + np.cos(np.pi * e / es)) * args.base_lr + args.min_lr
        return lr
...
def multistep_lr(args, batch_num):
    ...
    def _lr_adjuster(epoch):
        lr = args.base_lr * (args.lr_gamma ** (epoch / args.lr_adjust)) + args.min_lr
        return lr
...
def poly_lr(args, batch_num):
    learning_rate = []

    def _lr_adjuster(epoch):
        if epoch < args.warmup_length:
            coef = epoch / args.warmup_length
            coef *= (1 - args.warmup_length / args.epochs) ** args.poly_power
        else:
            coef = (1 - epoch / args.epochs) ** args.poly_power
        return coef * (args.base_lr - args.min_lr) + args.min_lr
